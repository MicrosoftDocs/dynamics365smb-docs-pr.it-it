...
description: 'Questo articolo fornisce alcune indicazioni per l''identificazione, la risoluzione dei problemi e la risoluzione di errori di sincronizzazione.'
author: brentholtorf
ms.topic: conceptual
ms.devlang: al
...
|La tua connessione a Dataverse è stata sospesa durante la sincronizzazione. Ad esempio, perché la sessione è scaduta. In tal caso, quando si riprende la sessione viene visualizzato il seguente errore: _La connessione alla tabella per il tipo di tabella CRM deve essere registrata utilizzando RegisterTableConnection o il cmdlet New-NAVTableConnection per poter essere utilizzata._|* Per i timeout della sessione, aggiorna la pagina per riconnetterti a Dataverse. Il messaggio di errore scompare.<br><br>* Per un problema nel codice, ad esempio, se utilizzi una pagina personalizzata per mostrare i dati di un'entità Dataverse , contatta il tuo partner o supporto Microsoft. Quando lo fai, usa l'azione **Copia i dettagli** per condividere i dettagli sull'errore. |
...
* I campi **Origine** e **Destinazione** possono contenere collegamenti alla riga in cui è stato trovato l'errore. Scegli il collegamento per indagare sull'errore.  
