---
title: Mapping le tabelle e i campi da sincronizzare
...
---
# <a name="mapping-the-tables-and-fields-to-synchronize"></a>Mapping le tabelle e i campi da sincronizzare
...
### <a name="add-table-and-field-mappings-to-existing-integration-tables"></a>Aggiungere mapping di tabelle e campi alle tabelle di integrazione esistenti

Nella pagina  **tabella di integrazione Mapping**, puoi utilizzare la nuova azione mapping della tabella per aggiungere nuovi mapping di tabella alle tabelle di integrazione. L'azione avvia la configurazione assistita  **Crea nuovi mapping di integrazione** guida, che aiuta a configurare nuovi mapping di campo e tabella di integrazione.

Guida consente di mappare le tabelle in [!INCLUDE [prod_short](includes/prod_short.md)] con le tabelle di integrazione esistenti Dataverse. È inoltre possibile specificare l' Dataverse identificatore univoco della tabella in **tabella di integrazione Nome campo identificatore univoco** e **tabella di integrazione Nome campo modificato**, necessari per il funzionamento della sincronizzazione dei dati.

> [!NOTE]
> La ricerca nella colonna Tabella mostra tutte le tabelle, comprese quelle personalizzate, che non sono ancora state mappate. [!INCLUDE [prod_short](includes/prod_short.md)]  Esclude quelli già presenti nella pagina dei mapping tabella di integrazione. La ricerca nella colonna tabella di integrazione mostra solo le tabelle (tabelle proxy AL) che fanno parte delle integrazioni pronte all'uso. Dataverse  Per aggiungere tabelle aggiuntive o personalizzate, avrai bisogno dell'aiuto di uno sviluppatore per crearle e distribuirle tramite un'estensione. Dataverse 

L'azione Avanzata apre impostazioni aggiuntive mapping della tabella che puoi configurare. Nella sezione  **Avanzate**, il campo  **Filtro tabella** consente di specificare i filtri per [!INCLUDE [prod_short](includes/prod_short.md)]. Utilizzare il campo  **Filtro tabella di integrazione** per i dati da considerare nella sincronizzazione dei dati insieme ai codici del modello di configurazione in  Dataverse Codice modello di configurazione tabella **e** Codice di configurazione tabella int. **per precompilare i dati mancanti quando nuovi dati provengono da una delle due applicazioni.** 

Guida ti aiuta anche a specificare i campi che vuoi che contenga il nuovo mapping. È possibile abbinare un campo nella colonna  [!INCLUDE [prod_short](includes/prod_short.md)] Nome campo **Nome campo** con un nome campo nella colonna  Dataverse Nome campo integrazione **.**  È inoltre possibile specificare una **Direzione**, se il campo ha sempre lo stesso valore nella colonna **Costante**  e se è necessario applicare convalide o trasformazioni durante il campo mapping.

> [!NOTE]
> Per ridurre gli errori, la ricerca della colonna  **Campo tabella di integrazione** viene filtrata in un elenco di campi disponibili. Per mappare i campi non inclusi nei filtri, avrai bisogno dell'aiuto di uno sviluppatore.
>
> * I campi devono essere abilitati.
> * Non è possibile utilizzare guida per mappare i campi FlowField e FlowFilter.
...
> [!TIP]
> È inoltre possibile aggiungere nuove mappature di campo alle mappature tabella di integrazione esistenti tramite la pagina elenco  **Mappature campi di integrazione**, a cui è possibile accedere scegliendo  **Campi** nella pagina  **Mappature tabella di integrazione** .
>
> Basta scegliere **Nuovo campo mapping** per aprire il campo mapping passaggio della configurazione assistita **Crea nuovi mapping di integrazione** guida, quindi procedere come descritto in questo articolo.

Le nuove mappature dei campi aggiunte a un mapping della tabella esistente sono in stato Disabilitato. Puoi scegliere **Modifica elenco** per abilitarli in un secondo momento.
...
### <a name="handle-differences-in-field-values"></a>Gestione delle differenze nei valori di campo
...
### <a name="handle-missing-option-values"></a>Gestione dei valori delle opzioni mancanti
...
## <a name="couple-records"></a>Associare i record
...
## <a name="filter-records-and-rows"></a>Filtro di record e righe
...
## <a name="create-new-records"></a>Creare nuovi record
