...
---
# <a name="depreciate-or-amortize-fixed-assets"></a>Ammortizzare cespiti
...
Una volta al mese, oppure ogniqualvolta sia necessario, è possibile eseguire il processo batch **Calcola Ammortamento**. Il processo non considera i cespiti che sono stati venduti, bloccati o inattivi né i cespiti che utilizzano il metodo di ammortamento manuale.  
...
## <a name="to-calculate-allocations-for-a-fixed-asset"></a>Per calcolare le allocazioni per un cespite
...
2. Scegli **Correlato**, **Categoria registrazione**, **Allocazioni**, quindi seleziona **Ammortamento**.
3. Compila i conti C/G e le percentuali di allocazione corrispondenti per ciascuna categoria di registrazione, quindi chiudi la pagina.
4. Scegli quindi l'icona ![lampadina che apre la funzione Dimmi.](media/ui-search/search_small.png "Informazioni sull'operazione che si desidera eseguire") immetti **Registrazioni cespiti in C/G**, quindi scegli il collegamento correlato.
5. Immetti il cespite e la corrispondente categoria di registrazione cespite e il registro beni ammortizzabili determineranno le righe di allocazione.
6. Scegliere l'azione **Registra** per eseguire la registrazione.

> [!NOTE]  
> Se un cespite viene utilizzato da più reparti, è possibile selezionare la dimensione nella riga Registrazioni cespiti in C/G.
