---
title: Utilizzare le dimensioni per tracciare e analizzare i dati
...
Un set di dimensioni è una combinazione univoca di valori dimensioni. Vengono memorizzati nel database come voci set di dimensioni. Ogni movimento set di dimensioni rappresenta un valore dimensioni singolo. Inoltre, ogni set di dimensioni e il movimento set di dimensioni al suo interno sono identificati da un ID set di dimensioni comune.  
...
## <a name="set-up-dimensions"></a>Imposta dimensioni
...
* **Dimensioni globali** utilizzate come filtro, ad esempio, per report, processi batch e report XML. Puoi utilizzare solo due dimensioni globali, quindi scegli le dimensioni che utilizzerai più spesso.
...
3. Compilare i campi per ogni nuova dimensione che si desidera impostare. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)]
...
Per evitare di registrare movimenti con dimensioni contraddittorie o non pertinenti è possibile bloccare o limitare specifiche combinazioni di due dimensioni. Una combinazione di dimensioni bloccata significa che non è possibile registrare entrambe le dimensioni nella stessa voce, indipendentemente dai valori delle dimensioni. Una combinazione dimensioni limitata permette invece di registrare entrambe le dimensioni nello stesso movimento, ma soltanto per determinate combinazioni di valori dimensioni.
...
2. Nella pagina **Combinazioni di dimensioni** seleziona il campo della combinazione dimensioni desiderato tra le seguenti opzioni.  
    ...
    |**Bloccato**|Questa combinazione di dimensioni non è consentita.|  
...
3. Nella parte superiore della pagina, seleziona una delle due seguenti modalità per eseguire il processo batch.
    ...
    |Opzione|Descrizione|
    |-|-|
    |**Sequenziale**|(Predefinita) La modifica viene eseguita in una transazione che reimposta tutti i movimenti sulle dimensioni precedenti alla modifica.<br /><br />Questa opzione è consigliata se la società ha relativamente pochi movimenti registrati, nel qual caso il processo batch richiede il tempo più breve per essere completato. Il processo blocca più tabelle e blocca altri utenti finché non è Fatto. Si noti che con database di grandi dimensioni il processo potrebbe non essere completato in questa modalità. In tal caso, utilizzare l'opzione **Parallela**.|
    |**Parallela**|La modifica della dimensione avviene in più sessioni in background e l'operazione viene suddivisa in più transazioni. Per utilizzare questa opzione, attiva l'interruttore **Elaborazione parallela**. <br /><br />Consigliamo questa opzione per database di grandi dimensioni o aziende con numerose voci pubblicate perché verrà completata nel più breve tempo possibile. Si noti che in questa modalità il processo di aggiornamento non verrà avviato se è presente più di una sessione di database attiva.|  
