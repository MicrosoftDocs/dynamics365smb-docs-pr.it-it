...
Il livello più alto è rappresentato dal reparto di produzione. Le aree di produzione vengono assegnate a gruppi di aree di produzione. Ogni area di produzione può far parte di un solo reparto di produzione.

Alle aree di produzione è possibile assegnare diversi centri di lavoro. Un centro di lavoro può far parte di una sola area di produzione.  

La capacita pianificata di un'area di produzione consiste della disponibilità dei centri di lavoro corrispondenti oltre che della disponibilità pianificata dell'area stessa. La disponibilità pianificata di un reparto di produzione è data quindi dalla somma di tutte le disponibilità delle relative aree di produzione e dei relativi centri di lavoro. La disponibilità viene memorizzata nei movimenti di calendario.  
...
1. Scegli l'icona ![lampadina che apre la funzione Dimmi.](media/ui-search/search_small.png "Dimmi cosa vuoi fare") immetti **Aree di produzione**, quindi scegli il collegamento correlato.  
2. Scegli l'azione **Nuovo**.  
3. Compila i campi come necessario. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)]
4. Nel campo **Cod. gruppo aree prod.** selezionare il raggruppamento di risorse di livello più alto in base a cui è organizzata l'area di produzione, se pertinente. Scegliere l'azione **Nuovo** nell'elenco a discesa.  
...
10. Nel campo **Calcolo costo unitario** specificare se basare il calcolo del costo unitario sulla quantità di tempo utilizzata o sul numero di unità prodotte.  
11. Selezionare il campo **Costo unitario specifico** se si desidera definire il costo unitario dell'area di produzione nella riga ciclo in cui viene utilizzato. Tale impostazione potrebbe risultare pertinente per le operazioni con costi delle capacità diversi rispetto a quelli consueti per l'area di produzione.  
12. Nel campo **Metodo consuntivazione** specificare se calcolare e registrare l'output nell'area di produzione manualmente o automaticamente, mediante uno dei metodi seguenti.
    ...
    |**Da Fine Ordine**|L'output viene registrato automaticamente quando l'ordine di produzione risulta finito.|
...
13. Nel campo **Cod. Unità di Misura** immettere l'unità di tempo utilizzata per il calcolo del costo dell'area di produzione specificata e per la programmazione della capacità.
    ...
    > [!NOTE]  
    > Se utilizzi Giorni, ricorda che un giorno equivale a 24 ore e non a una giornata lavorativa di otto ore.

14. Il campo **Capacità** consente di specificare se nell'area di produzione sono disponibili più macchinari o persone che lavorano contemporaneamente. Se [!INCLUDE[prod_short](includes/prod_short.md)] non include la funzionalità Centro di lavoro, il valore in questo campo deve essere **1**.  
...
16. Attiva l'interruttore **Calendario consolidato** se si utilizzano anche centri di lavoro. Questa impostazione assicura il roll up dei movimenti di calendario dai calendari centro di lavoro.  
...
Quando si assegna una riga del ciclo di produzione a un'area di produzione o a un centro di lavoro, [!INCLUDE [prod_short](includes/prod_short.md)] calcola:
...
- Un'efficienza dell'80% significa che avrai bisogno di 2,5 ore invece di due ore  
- Un'efficienza del 200% significa che puoi completare il lavoro in un'ora. Puoi scavare la buca due volte più velocemente se hai un escavatore grande il doppio di quello più piccolo  

    Puoi ottenere lo stesso risultato se utilizzi due escavatori più piccoli invece di uno grande. Usa *2* come per la capacità e *100%* come l'efficienza  
...
- Un'efficienza dell'80% significa che avrai bisogno di 2,5 ore invece di due ore per il setup.  
- Un'efficienza del 200% significa che puoi completare il setup in 1 ora invece di 2 ore definite nella riga del ciclo di produzione.  
...
In questo caso, il costo allocato agli ordini è gestito dal tempo di setup e dalla capacità simultanea. Si consiglia di non utilizzare il tempo di lavorazione nelle righe del ciclo di produzione.  
...
- Se sono presenti due ordini, la capacità simultanea nella riga del ciclo di produzione deve essere impostata su 0,5.
    ...
    Di conseguenza, la capacità assegnata per ciascuno è di un'ora, ma la durata per ogni ordine rimane di due ore.
- Se sono presenti due ordini con una quantità rispettivamente di uno e quattro, la capacità simultanea per la riga del ciclo di produzione del primo ordine è 0,2 e 0,8 per il secondo.  
    ...
    Di conseguenza, la capacità assegnata per il primo ordine è di 24 minuti e per il secondo 96. La durata per entrambi gli ordini rimane di due ore.  
...
Ad esempio, se disponi di due centri di lavoro con un'efficienza rispettivamente di 80 e 70, la voce del calendario consolidato ha:

- Un'efficienza pari a 100
- Una capacità di 1,5
- Una capacità totale di 12 ore (turno di otto ore * capacità di 1,5).
...
Se vengono assegnati diversi centri di lavoro (ad esempio, 210 tavolo da imballaggio 1, 310 cabina verniciatura...) a un'area di produzione, risulta importante prendere in considerazione le singole capacità dei centri di lavoro in quanto il mancato funzionamento di uno di essi può interrompere l'intero processo. I gruppi di macchine possono essere immessi in base alla loro capacità, ma non vengono inclusi nella pianificazione. Se disattivi l'interruttore **Calendario consolidato** solo la capacità dell'area di produzione viene assegnata nella pianificazione. La capacità del centro di lavoro viene esclusa.

Se, tuttavia, si combinano centri di lavoro identici (ad esempio, 210 tavolo da imballaggio 1 e 220 tavolo da imballaggio 2) in un'area di produzione, è possibile considerare l'area di produzione come la somma dei centri di lavoro assegnati. L'area di produzione viee pertanto elencata con capacità pari a zero. Se si attiva l'interruttore **Calendario consolidato**, all'area di produzione viene assegnata la capacità standard.
...
Nella pagina **Risorse critiche**, è possibile creare un setup per evitare che le risorse risultino sovraccaricate. Il setup può anche assicurare che la capacità non venga allocata se può comportare un aumento del tempo di completamento di un ordine di produzione. Nel campo **Smorzamento (% cap. totale)**, è possibile aggiungere il tempo di smorzamento alle risorse per ridurre al minimo la suddivisione dell'operazione. Questa impostazione consente al sistema di pianificare il carico nell'ultimo giorno possibile superando leggermente la percentuale di carico critico. Il superamento della percentuale di carico può ridurre il numero di operazioni suddivise.

Nella pianificazione con risorse vincolate alla capacità, [!INCLUDE [prod_short](includes/prod_short.md)] garantisce che nessuna risorsa venga caricata oltre la propria capacità definita (carico critico). Assegna ogni operazione alla fascia oraria disponibile più vicina. Se la fascia oraria non è sufficiente a completare l'intera operazione, l'operazione viene divisa in due o più parti e le colloca nelle fasce orarie adiacenti disponibili.

1. Scegli l'icona ![lampadina che apre la funzionalità Dimmi.](media/ui-search/search_small.png "Dimmi cosa vuoi fare") immetti **Risorse critiche**, quindi scegli il collegamento correlato.
2. Scegliere l'azione **Nuovo**.
3. Compilare i campi come necessario.

> [!NOTE]
> Le operazioni nelle aree di produzione o nei centri di lavoro impostati come risorse vincolate vengono sempre pianificate in modo seriale. Questa pianificazione significa che se una risorsa vincolata ha più capacità, tali capacità devono essere pianificate in sequenza. Non è possibile pianificarle in parallelo come lo sarebbero se il centro di lavoro o l'area di produzione non fosse impostato come risorsa vincolata. Per una risorsa vincolata, il campo **Capacità** nell'area di produzione o nel centro di lavoro è maggiore di **1**.
