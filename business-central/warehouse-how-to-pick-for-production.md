---
title: 'Preleva o sposta articoli per la produzione, l''assemblaggio o progetti in configurazioni di magazzino di base'
...
---
# <a name="pick-for-production-assembly-or-projects-in-basic-warehouse-configurations"></a>Scegli per produzione, assemblaggio o progetti in configurazioni di magazzino di base
...
In una configurazione di magazzino di base per il flusso in uscita (prelievo), nella pagina  **Posizione scheda** per la posizione, attivare le seguenti impostazioni:

* Produzione, Seleziona **Prelievo/Movimento inventario** nel campo **Prod. Consumo Whse. Movimentazione** .
* Assemblaggio, Seleziona **Movimento inventario** nel campo **Asm. Consumo Whse. Movimentazione** .
* Gestione del progetto, Seleziona **Prelievo inventario** nel campo **Consumo progetto Gestione magazzino** .
...
* Quando si registra un prelievo di inventario per un'operazione interna, come una produzione o un progetto, il consumo dei componenti prelevati viene registrato contemporaneamente.
...
Per le ubicazioni che utilizzano contenitori, è possibile estendere il flusso al progetto con documenti  **Movimento inventario** .

> [!NOTE]
> [!INCLUDE[prod_short](includes/prod_short.md)] utilizza il valore del campo **Quantità rimanente** nella riga di pianificazione progetto quando crea prelievi magazzino. Per utilizzare le selezioni di inventario per i progetti, è necessario attivare l'opzione  **Applica utilizzo collegare** nella pagina  **Progetto scheda** per i progetti. Ciò ti consente di monitorare l'utilizzo rispetto al tuo piano. Se non si attiva l'interruttore, la quantità rimanente rimarrà a **0** e il prelievo magazzino non verrà creato. Per ulteriori informazioni, vedi [Per impostare la tracciabilità dell'utilizzo di un progetto](projects-how-setup-jobs.md?tabs=current-experience#to-set-up-project-usage-tracking).
...
1. Nel documento di origine, che può essere un ordine di produzione o un progetto, selezionare l'azione  **Crea stoccaggio/prelievo inventario** .  
...
1. Nel documento di origine, che può essere un ordine di produzione, un ordine di assemblaggio o un progetto, selezionare l'azione  **Crea stoccaggio/prelievo inventario** .  
...
|Progetti | Modifica lo stato di un progetto in  **Aperto** oppure crea subito un progetto con stato Aperto.|  
