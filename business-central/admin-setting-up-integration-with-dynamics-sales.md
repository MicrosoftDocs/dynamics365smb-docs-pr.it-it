...
---
# <a name="setting-up-user-accounts-for-integrating-with-microsoft-dataverse-via-data-sync"></a>Impostazione di account utente per l'integrazione con Microsoft Dataverse tramite la sincronizzazione dei dati
...
> [!IMPORTANT]
> Durante l'installazione, ti verrà chiesto di fornire le credenziali per l'ambiente [!INCLUDE[prod_short](includes/cds_long_md.md)]. Fornire le credenziali di un account che sia un utente con licenza e assegnato al ruolo di sicurezza  **Amministratore di sistema** su  [!INCLUDE[prod_short](includes/cds_long_md.md)] ambiente e al ruolo di  [Amministratore utente](/entra/identity/role-based-access-control/permissions-reference#user-administrator) sul tenant a cui appartiene ambiente. Questo account non ha bisogno di una licenza per [!INCLUDE[prod_short](includes/prod_short.md)], in quanto verrà utilizzato solo per eseguire attività di configurazione nell'ambiente [!INCLUDE[prod_short](includes/cds_long_md.md)].
>
...
La soluzione di integrazione di base crea il seguente ruolo in [!INCLUDE [cds_long_md](includes/cds_long_md.md)] per l'integrazione:

* **Integrazione Dataverse in Business Central**: consente di gestire la connessione tra [!INCLUDE [prod_short](includes/prod_short.md)] e [!INCLUDE [cds_long_md](includes/cds_long_md.md)]. Assegnato in genere solo all'account utente automaticamente creato per la sincronizzazione.

> [!NOTE]
> Solo l'utente dell'applicazione che esegue l'integrazione deve avere il ruolo **Integrazione Dataverse di Business Central**. L'utente dell'applicazione non ha bisogno della licenza [!INCLUDE [prod_short](includes/prod_short.md)] o delle licenze [!INCLUDE [cds_long_md](includes/cds_long_md.md)].
